...
impl RateLimiter {
    ...
    pub fn check_rate_limit(
        ...
        if Self::is_whitelisted(env, user, endpoint)? {
            return Ok(true);
        }

        // Check if user is blocked by DDoS protection
        if Self::is_ddos_blocked(env, user, current_time)? {
        ...
        for limit in limits.iter() {
            if Self::is_limit_applicable(&limit, user, endpoint) {
                if !Self::check_single_limit(env, &limit, user, endpoint, current_time)? {
                    Self::update_metrics(env, 1, 1, 0, 0)?;
                    
                    // Check for potential DDoS
                    Self::check_ddos_violation(env, user, current_time)?;
    ...
    fn record_request(
        ...
        let user_type_map = user_requests.get(user)
            .unwrap_or_else(|| Map::new(env));
        ...
        for limit_type in [RateLimitType::PerSecond, RateLimitType::PerMinute, RateLimitType::PerHour, RateLimitType::PerDay] {
            let mut requests = user_type_map.get(limit_type)
                .unwrap_or_else(|| Vec::new(env));
            ...
            let mut updated_type_map = user_type_map;
            updated_type_map.set(limit_type, filtered_requests);
            user_requests.set(user, updated_type_map);
        }
        ...
        if let Some(endpoint_sym) = endpoint {
            ...
            let user_map = endpoint_requests.get(endpoint_sym)
                .unwrap_or_else(|| Map::new(env));
            
            let mut requests = user_map.get(user)
                .unwrap_or_else(|| Vec::new(env));
            requests.push_back(current_time);
            ...
            let mut updated_user_map = user_map;
